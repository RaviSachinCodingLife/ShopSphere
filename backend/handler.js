...
const customers = ["Ava","Liam","Mia","Noah","Zoe","Ivy","Ethan","Mila","Aria","Kai"];
...
export const resolvers = {
  Query: {
    ...
    dashboardKPIs: () => ({
      ...
      ordersInProgress: ORDERS.filter((o) => o.status === "PENDING" || o.status === "SHIPPED").length,
    ...
    salesAnalytics: (_, { range, groupBy, category, region }) => {
      ...
      const revenueTrend = Array.from({ length: groups }).map((_, i) => {
        ...
        return { date: d.toISOString().slice(0, 10), value: Math.round(2000 + Math.random() * 5000) };
      });
    ...
    userBehavior: () => {
      ...
      return { activeUsers, newUsers, returningUsers, avgSessionMins: +(5 + Math.random() * 4).toFixed(1), funnel };
    },

    orders: (_, { first, after, status }) => {
      let list = [...ORDERS];
      if (status) list = list.filter((o) => o.status === status);
      const start = after ? Math.max(0, list.findIndex((o) => o.id === after) + 1) : 0;
      const slice = list.slice(start, start + first);
      const edges = slice.map((o) => ({ node: o, cursor: o.id }));
      return { edges, pageInfo: { endCursor: edges.length ? edges[edges.length - 1].cursor : null, hasNextPage: start + first < list.length } };
    },
  ...
  Subscription: {
    orderUpdated: { subscribe: () => { throw new Error("Subscriptions require WebSocket infra; add API Gateway WS in prod."); } },
  },
};
